...
;Project: Assembly Decoder (Lab02)
;Author: John Paul Terragnoli
;Period: T5A
;Teacher: Captain Trimble
;Date Started: 16 SEP 14
;Date Finished: 21 SEP 14
;Description: Intakes bytes from ROM (0xC000) that serve as a decoded message.  The program then
;decodes the message using a given key and XORing and writes it in memory (0x0200).
;Inputs: Key, Marker, and Message from ROM (0xC000)
;Outputs: Decoded message in memory (0x0200)
;Registers Destroyed: r4-r11
;-------------------------------------------------------------------------------
;
;
;-------------------------------------------------------------------------------
...
;           location.
;Inputs: mess_loc, write_loc, decryptCharacter (inputs for this can be seen below), #end
;Outputs:	mess_loc, write_loc
;Registers destroyed: check_end, mess_loc, write_loc
;-------------------------------------------------------------------------------
...
;Inputs:	key_length, key_counter, mess_loc, key_part, #key_loc, #ZER0_W
;Outputs:	key_counter, key_part, 0(write_loc)
;Registers destroyed: holder, decryp_reg
;-------------------------------------------------------------------------------
...
;is not used in any of the keys or the messages for the test cases.  Also, this subroutine
;finds the location of the beginning of the key.
;Inputs:	#key_loc, #ZERO_W, #beginning,
;Outputs:	key_part, key_length, mess_loc, key_counter
